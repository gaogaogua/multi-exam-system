...
const UIController = {
  ...
  showQuestionDetail(id) {
    ...
    Modal.open({
      id: 'detail', title: '题目详情', size: '',
      body: `<p style="margin-bottom:8px;line-height:1.8;">${Utils.escapeHtml(q.title)}</p>
  ...
  showAddQuestionModal(editId) {
    ...
    if (isEdit) { const e = QuestionBank.getById(editId); if (e) q = e; }
    ...
    Modal.open({
      id: 'add-question', title: isEdit ? '编辑题目' : '添加题目', size: 'lg',
      body: `<div class="form-group"><label>题型</label><select id="q-type" onchange="UIController.onTypeChange()">${['single','multiple','judge','fill','essay'].map(t => `<option value="${t}" ${q.type===t?'selected':''}>${App.getTypeName(t)}</option>`).join('')}</select></div>
  ...
  showAiProgress(total) {
    Modal.open({
      id: 'ai-progress', title: 'AI 智能解析进度', closable: false, backdrop: false,
      body: `<div style="text-align:center;padding:20px;"><p>AI 智能解析中...</p><p style="font-size:13px;color:var(--text-secondary);">共 ${total} 题</p><div id="ai-progress-list" style="max-height:300px;overflow-y:auto;margin-top:12px;text-align:left;"></div></div>`,
    });
  },
  ...
  closeAiProgress() { Modal.close('ai-progress'); },
};
